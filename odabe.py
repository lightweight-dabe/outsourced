#! /usr/bin/python3
'''
DABE Author: Gary Belvin
ODABE Authors: Mohammed B. M. Kamel, Janneke van Oosterhout
March 2023
'''
...
class singlenodev1():
    ...
    def publickeygenDecrypt(self, i, private_p):
        '''
        This procedure is related to the single node decryption model.
        Each attribute authority gets p and computes two values.
        These two values later are used to blind the calculations in the computational node.
        '''
        p = private_p[i.upper()]['p']
        key_gid = private_p[i.upper()]['key']
        h_gid = private_p[i.upper()]['hash']

        private_p[i.upper()]['key^p'] = (key_gid ** p)
        private_p[i.upper()]['hash^p'] = (h_gid ** p)

    def prepare_decrypt_parameters(self, GP, private_p, ct, sk):
        '''Prepare the required parameters for the decryption ... Done by the decryptor node'''
        ...
        keys = {}
        # raise ciphertext C1, hash of the gid, and keys to power p, to blind them for computational node
        for pr in pruned:
            x = pr.getAttributeAndIndex()
            keys[x] = private_p[x]['key^p']
            p = private_p[x]['p']
            hash_gid = private_p[x]['hash^p']
            ct['C1'][x] = (ct['C1'][x])**p
        
        return {'p': p}, {'ct_p': ct, 'key_p': keys, 'hash_gid': hash_gid}
    
    def pre_decrypt(self, GP, public_parameters, ct):
        '''
        Pre Decrypt ... Done by the computational node
        The computational node does a pre decrypt with similar calculations to the normal decryption, but with received blinded values
        '''
        policy = util.createPolicy(ct['policy'])
        coff = util.getCoefficients(policy)
        E = 1

        ct_p = public_parameters['ct_p']
        keys = public_parameters['key_p']
        hash_gid = public_parameters['hash_gid']

        # computations of computational node
        for attr in keys.items():
            y = attr[0]
            num = ct_p['C1'][y] * pair(hash_gid, ct_p['C3'][y])
            dem = pair(keys[y], ct_p['C2'][y])
            E *= ( (num / dem) ** coff[y] )

        return E 

    def decrypt(self, GP, E, private_parameters, public_parameters):
        '''
        Decrypt ... Done by the decryptor node
        The final step of decryption. Prior to this algorithm, the (prepare_decrypt_parameters) and (pre_decrypt) should be already done
        '''
        p = private_parameters['p']
        ct_p = public_parameters['ct_p']
        E_p = E ** (1/p)
...
def main():   
    ...
    auth1_attrs = ['ONE', 'TWO', 'THREE', 'FOUR', 'FIVE', 'FIVE_0', 'FIVE_1', 'SIX', 'SEVEN']   # five_0 and five_1 in case the attribute five used twice in the access structure
    ...
    alice_attrs = ['ONE', 'TWO', 'THREE', 'FIVE', 'SIX']
    ...
    # Decryption ODABE ALICE
    private_p = {}
    p = group.random()
    for i in range(len(alice_attrs)):
        private_p[alice_attrs[i]] = {}
        private_p[alice_attrs[i]]['p'] = p
        private_p[alice_attrs[i]]['key'] = alice_sk[alice_attrs[i]]['k'] 
        private_p[alice_attrs[i]]['hash'] = GP['H'](alice_sk['gid']) 
    for i in alice_attrs: singlenode.publickeygenDecrypt(i, private_p)

    # ------------------------------------------------------  original decryption ALICE
    if dabe_execution:
        # Alice tries to decrypt the ciphertext
        if verbose: print("\n\n[x] Decrypt the random key by Alice...")
        original_decrypt_start_time_A = time.monotonic()
        alice_rec_key = decentralizedabe.original_decrypt(GP, alice_sk, CT)
        original_decrypt_end_time_A = time.monotonic()
    # ------------------------------------------------------  lightweight decryption ALICE
    if light_execution:
        notRightAttr = False
        preparation_start_time_dec = time.monotonic()
        private_parameters_dec, public_parameters_dec = singlenode.prepare_decrypt_parameters(GP, private_p, CT, alice_sk) 
        preparation_end_time_dec = time.monotonic()
        if private_parameters_dec == None:
            alice_rec_key = 0
            notRightAttr = True
        else:
            predecrypt_start_time = time.monotonic()
            E = singlenode.pre_decrypt(GP, public_parameters_dec, CT) 
            predecrypt_end_time = time.monotonic()
            decrypt_start_time = time.monotonic()
            alice_rec_key = singlenode.decrypt(GP, E, private_parameters_dec, public_parameters_dec) 
            decrypt_end_time = time.monotonic()
    # ------------------------------------------------------
    ...
    original_decrypt_start_time_B = time.monotonic()
    bob_rec_key = decentralizedabe.original_decrypt(GP, bob_sk, CT)
    original_decrypt_end_time_B = time.monotonic()
    ...
    if dabe_execution: 
        ...
        time_dabe_dec_file = open('dabe_time_decrypt.txt', 'a')
        time_dabe_dec_A = timedelta(seconds=(original_decrypt_end_time_A - original_decrypt_start_time_A))
        #time_dabe_dec_B = timedelta(seconds=(original_decrypt_end_time_B - original_decrypt_start_time_B))
        if verbose: print("[x] Execution Time (original DABE) decrypt Alice \t: ", time_dabe_dec_A)
        #if verbose: print("[x] Execution Time (original DABE) decrypt Bob \t: ", time_dabe_dec_B)
        time_dabe_dec_file.write(str(time_dabe_dec_A).split(":")[-1]+ " "+access_structure+ "\n")
        #time_dabe_dec_file.write(str(time_dabe_dec_B).split(":")[-1]+" not able to decrypt"+"\n")
        time_dabe_dec_file.close()
    if light_execution and not notRightAttr:
        ...
        time_odabe_dec_file = open('odabe_decryptor_time.text', 'a')
        time_odabe_dec = timedelta(seconds= (preparation_end_time_dec - preparation_start_time_dec) + (decrypt_end_time - decrypt_start_time))
        if verbose: print("[x] Execution Time (ODABE - decryptor) \t: ", time_odabe_dec)
        time_odabe_dec_file.write(str(time_odabe_dec).split(":")[-1]+ " "+access_structure +"\n")
        time_odabe_dec_file.close()
        if verbose: print("[x] Execution Time (ODABE - computational node) decryption : ", timedelta(seconds= (predecrypt_end_time - predecrypt_start_time)))
